...
export default function Component({ isOpen, points, onClose }: ComponentProps) {
  ...
  function shareOnX() {
    ...
    const landingUrl = `https://ifsci.wtf/static/share/twitter-share-landing.html?t=${timestamp}`
    const url = `https://x.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(landingUrl)}`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div
        className="relative max-h-[365px] w-full max-w-[386px] rounded-3xl bg-[#1A1A1A] p-6 text-center"
        style={{
          backgroundImage: `url(${modalBackgroundPng})`,
