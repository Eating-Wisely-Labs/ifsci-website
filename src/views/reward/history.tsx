...
const RewardHistory: React.FC = () => {
  ...
  const { records, total, pageSize } = useRewardStore()
  ...
  useEffect(() => {
    ...
    setLoading(true)
    rewardStoreActions
      .getRewardList(publicKey.toString(), page)
      .then((data) => {
              ...
              <>
                <div className="mb-6 overflow-hidden rounded-lg bg-white/5 backdrop-blur">
                  <div className="grid grid-cols-[1fr_1fr_auto] border-b border-white/10 px-6 py-4">
                    ...
                    <div className="text-sm text-white/60">Reward Points</div>
                  </div>
                  <div className="divide-y divide-white/10">
                    {records.map((record, index) => (
                      <div key={index} className="grid grid-cols-[1fr_1fr_auto] items-center px-6 py-4">
                        <div>{getFormattedDate(record.create_time)}</div>
                        <div>{record.reward_type_name}</div>
                        <div>
